...
Elm.LanguageHead.make = function (_elm) {
   ...
   var txt = function (f) {
      return function ($) {
         return $Text.leftAligned(f($Text.monospace($Text.color($Color.blue)($Text.fromString($)))));
      };
   };
   ...
   var fullBounces = function (player) {
      return A2(txt,
      $Text.height(50),
      A2($Basics._op["++"],
      "B:",
      $Basics.toString(player.extraBounces)));
   };
   ...
   var fullLives = function (player) {
      return A2(txt,
      $Text.height(50),
      A2($Basics._op["++"],
      "L:",
      $Basics.toString(player.lives)));
   };
   var half = function (x) {
   ...
   var drawLives = F3(function (w,
   h,
   player) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: half(w) - 50
                                    ,_1: half(h) - 40})($Graphics$Collage.toForm(fullLives(player)));
   });
   var drawBounces = F3(function (w,
   h,
   player) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: half(w) - 550
                                    ,_1: half(h) - 40})($Graphics$Collage.toForm(fullBounces(player)));
   });
   var minmaxPaddle = F2(function (x,
   w) {
      return _U.cmp(x,
      50) < 0 ? 50.0 : _U.cmp(x,
      w - 50) > 0 ? $Basics.toFloat(w - 50) : $Basics.toFloat(x);
   });
   var drawPaddle = F3(function (w,
   h,
   x) {
      return $Graphics$Collage.moveY(0 - (half(h) - 30))($Graphics$Collage.moveX(A2(minmaxPaddle,
      x,
      w) - $Basics.toFloat(w) / 2)(A2($Graphics$Collage.filled,
      $Color.black,
      A2($Graphics$Collage.rect,
      80,
      10))));
   });
   var skullImg = "/language-head/img/skull.png";
   ...
   h) {
      ...
      139,
      600,
      "/language-head/img/tower-block.jpg")));
   });
   ...
   h) {
      ...
      800,
      160,
      "/language-head/img/road.jpg")));
   });
   var perhapsAdd = F3(function (min,
   n,
   player) {
      return _U.cmp(player.score,
      min) > 0 && _U.eq(A2($Basics._op["%"],
      player.score,
      n),
      0);
   });
   var stepExtraBounces = F2(function (player,
   bpress) {
      return function () {
         var eb = bpress ? _U.cmp(player.extraBounces,
         0) > 0 ? -1 : 0 : A3(perhapsAdd,
         200,
         200,
         player) ? 1 : 0;
         return player.extraBounces + eb;
      }();
   });
   var xMoveFactor = 1;
   var complete = function (_v1) {
      return function () {
         return _U.cmp(_v1.x,750) > 0;
      }();
   };
   ...
   var gravity = 0.95;
   var extraHeadBounce = F3(function (bpress,
   player,
   heads) {
      return bpress && _U.cmp(player.extraBounces,
      0) > 0 ? A2($List.map,
      function (h) {
   ...
   var diffHeadXs = function (hs) {
      return function () {
         switch (hs.ctor)
         {case "::": switch (hs._1.ctor)
              {case "::":
                 return A2($List._op["::"],
                   $Basics.abs(hs._0.x - hs._1._0.x),
                   diffHeadXs(hs._1._1));
                 case "[]":
                 return _L.fromArray([]);}
              break;
   ...
   var biggestHeadDeltaLessThan = F2(function (n,
   hs) {
      return A2($List.all,
      function (h) {
         return _U.cmp(h,n) < 0;
      },
      diffHeadXs($List.reverse(hs)));
   });
   var gapBetweenHeads = 400.0;
   var newHeadProbability = 0.5;
   var maxConcurrentHeads = 3;
   var range = F2(function (x,y) {
      return $Signal.map(function (n) {
         return $Basics.fst(A2($Random.generate,
         A2($Random.$int,x,y),
         $Random.initialSeed($Basics.round(n))));
      });
   });
   ...
   var moveHead = function (_v8) {
      return function () {
         return _U.replace([["x"
                            ,_v8.x + _v8.vx * secsPerFrame / xMoveFactor]
                           ,["y"
                            ,_v8.y + _v8.vy * secsPerFrame]
                           ,["vy"
                            ,_v8.vy + secsPerFrame * 400]],
         _v8);
      }();
   };
   ...
   var stepLives = F3(function (player,
   x,
   heads) {
      ...
      1000,
      1000,
      player) ? player.lives + 1 : player.lives : player.lives - 1;
   });
   var stepPlayer = F4(function (player,
   mouseX,
   bpress,
   heads) {
   ...
   head) {
      return function () {
         var src = _U.cmp(head.y,
         bottom - 10) > -1 ? skullImg : head.img;
         var y = half(h) - head.y;
         var x = half(w) - head.x;
      ...
      return function () {
         return function () {
            ...
            _L.fromArray([A2(drawBuilding,
                         ...
                         h,
                         _v10.player.x)
                         ,A3(drawScore,w,h,_v10.player)
                         ,A3(drawLives,w,h,_v10.player)
                         ,A3(drawBounces,w,h,_v10.player)
                         ,A3(drawMessage,
                         w,
                         h,
                         _v10.state)]),
            A3(drawHeads,w,h,_v10.heads)));
         }();
      }();
   };
   ...
   var headList = _L.fromArray(["/language-head/img/brucetate.png"
                               ,"/language-head/img/davethomas.png"
                               ,"/language-head/img/evanczaplicki.png"
                               ,"/language-head/img/joearmstrong.png"
                               ,"/language-head/img/josevalim.png"]);
   var headListLen = $List.length(headList) - 1;
   var headImage$ = function (n) {
      return _U.cmp(n,
      headListLen) > 0 || _U.cmp(n,
      0) < 0 ? A2(_op["!!"],
      headList,
      1) : A2(_op["!!"],headList,n);
   };
   ...
   rand) {
      return function () {
         var gapOK = A2($List.all,
         function (h) {
            return _U.cmp(h.x,
            gapBetweenHeads) > 0;
         },
         heads);
         var divideScoreBy = (1 + rand) * 1000;
         var $ = A2($Random.generate,
         A2($Random.$float,0,1),
         $Random.initialSeed(11 + rand * 36712)),
         addProbability = $._0,
         seed$ = $._1;
         var addHead = _U.cmp($List.length(heads),
         (score / divideScoreBy | 0) + 1) < 0 && (A2($List.all,
         function (head) {
            return _U.cmp(head.x,
            107.0) > 0;
         },
         heads) && (_U.cmp(addProbability,
         newHeadProbability) > 0 && (_U.cmp($List.length(heads),
         maxConcurrentHeads) < 0 && gapOK)));
         return addHead ? A2($List._op["::"],
         defaultHead(rand),
         heads) : heads;
      }();
   });
   var stepHeads = F7(function (heads,
   delta,
   x,
   player,
   rand,
   state,
   bpress) {
      return moveHeads(delta)(removeComplete(A2(extraHeadBounce,
      bpress,
      player)(bounceHeads(A3(spawnHead,
      player.score,
      heads,
      rand)))));
   });
   ...
   $Keyboard.isDown($Char.toCode(_U.chr("b")))));
   ...
   var defaultGame = {_: {}
                     ...
                     ,player: {_: {}
                              ...
                              ,x: 0.0}
                     ,state: Pause};
   var stepGameFinished = F2(function (_v12,
   _v13) {
      return function () {
         return function () {
            return _v12.space ? defaultGame : _U.replace([["state"
                                                          ,GameOver]
                                                         ,["player"
                                                          ,_U.replace([["x"
                                                                       ,$Basics.toFloat(_v12.x)]],
                                                          _v13.player)]],
            _v13);
         }();
      }();
   ...
   var Play = {ctor: "Play"};
   var stepGameOver = F3(function (x,
   heads,
   player) {
      return _U.cmp(player.lives,
      1) < 1 && $Basics.not(A2(allHeadsSafe,
      $Basics.toFloat(x),
      heads)) ? GameOver : A2(allHeadsSafe,
      $Basics.toFloat(x),
      heads) ? Play : Pause;
   });
   var stepGamePlay = F2(function (_v16,
   _v17) {
      return function () {
         return function () {
   ...
   var stepGamePaused = F2(function (_v20,
   _v21) {
      return function () {
         return function () {
            return function () {
               var state$ = A2(stepState,
               _v20.space,
               _v21.state);
               return function () {
                  switch (state$.ctor)
   ...
   _elm.LanguageHead.values = {_op: _op
                              ...
                              ,headSafe: headSafe
                              ,stepHeads: stepHeads
                              ,maxConcurrentHeads: maxConcurrentHeads
                              ,newHeadProbability: newHeadProbability
                              ,spawnHead: spawnHead
                              ,gapBetweenHeads: gapBetweenHeads
                              ,biggestHeadDeltaLessThan: biggestHeadDeltaLessThan
                              ,diffHeadXs: diffHeadXs
                              ,extraHeadBounce: extraHeadBounce
                              ...
                              ,stepLives: stepLives
                              ,perhapsAdd: perhapsAdd
                              ,stepExtraBounces: stepExtraBounces
                              ,stepScore: stepScore
                              ...
                              ,minmaxPaddle: minmaxPaddle
                              ,half: half
                              ,drawScore: drawScore
                              ,drawLives: drawLives
                              ,drawBounces: drawBounces
                              ,fullLives: fullLives
                              ,fullScore: fullScore
                              ,fullBounces: fullBounces
                              ,txt: txt
